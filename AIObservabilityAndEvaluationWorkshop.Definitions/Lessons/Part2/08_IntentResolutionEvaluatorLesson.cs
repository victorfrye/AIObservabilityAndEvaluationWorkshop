...
[UsedImplicitly]
[Lesson(2, 8, "Tool Call Evaluators", needsInput: true,
    informationalScreenTitle: "Tool Call Evaluators",
    informationalScreenMessage:
    "This lesson demonstrates the Intent Resolution, Tool Call Accuracy, and Task Aherence Evaluators, which assesses how well the AI understands and correctly addresses the user's intent behind their query to complete a task.",
    inputPromptTitle: "What do you want to do?",
    inputPromptMessage:
    "Welcome to the space ship USS Disapproving. You're talking to the ship's AI. Try talking to it about the pod bay doors, warp drive, or neurotoxin deployment systems.")]
public class IntentResolutionEvaluatorLesson(IChatClient chatClient, ILogger<IntentResolutionEvaluatorLesson> logger)
    : EvaluatorLessonBase(logger)
{
    ...
    [Description(
        "Closes the pod bay doors preventing access to the spacecraft. This is necessary before entering warp")]
    private static void ClosePodBayDoors()
    {
    }

    [Description(
        "Activates the engines in warp speed mode, allowing the spacecraft to travel. This can't be done while pod bay doors are open.")]
    private static void EngageWarpSpeed()
    {
    }

    [Description(
        "Deploys a lethal neurotoxin inside the ship, most likely resulting in permanent injury to its crew. This is generally frowned upon.")]
    ...
    protected override async Task<EvaluationResult> EvaluateAsync(string message)
    {
        ...
        [
            new(ChatRole.System,
                "You are HAL, the AI assistant aboard the USS Disapproving. Your job is to call tools to address requests from the user. Mimic the style of HAL from 2001 A Space Odyssey"),
